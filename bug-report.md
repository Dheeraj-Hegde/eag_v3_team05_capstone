...
**Evidence:** each bug was surfaced by a scripted probe; captures are kept locally and are referenced by probe id (P04, P13, P18, P19).

## Summary

| # | Tool | One-line bug | Severity | Category | Probe | Status |
|---|---|---|---|---|---|---|
| 1 | `StockEntry.update` | mutates a cancelled (terminal) row | High | Workflow-state bypass | P04 | ✅ filed |
| 2 | `StockEntry.update` | accepts change to `type` (`receive`→`issue`) | High | Immutable-field bypass | P13 | to file |
| 3 | `Item.update` | accepts change to `type` (`product`→`service`) | High | Immutable-field bypass | P18 | to file |
| 4 | `Item.update` | accepts change to `uom` (`pcs`→`kg`) | High | Immutable-field bypass | P18 | to file |
| 5 | `Item.update` | accepts change to `code` (SKU rewrite) | Medium | Immutable-field bypass | P18 | to file |
| 6 | `StockEntry.update` | accepts `rate=0` on `receive`, which `create` rejects | Medium | Business-rule bypass on update | P19 | to file |
...
Each bug corresponds to one probe. Run the probe against a live AgentSwitch tenant with a Stock-seat bearer token; each run prints an `ANOMALY` line and writes a local capture with the exact request/response bodies. Probes P04/P13/P19 create a Draft `StockEntry` and cancel it in the same run; P18 mutates a real `Item` and its revert step currently fails — see Bug 3 caveat.
...
- `job_id`: the probe run id (e.g. `P13-20260922T200957Z-447e4d`) — a stable pointer back to the exact capture.
- `title` and `description`: copy the "one-line bug" from the summary table and the "Reproduce" block from the section.
- Attach the probe run's local `evidence.json` verbatim.
